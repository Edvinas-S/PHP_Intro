...
if ( !function_exists( 'cordero_dynamic_style' ) ) {
	function cordero_dynamic_style( $css = array() ) {
		...
		if ( $container_width && $container_width != '1240' ) {
			$css[] = '.container,.wp-block-cover-image.alignfull .wp-block-cover__inner-container,.wp-block-cover.alignfull .wp-block-cover__inner-container,.wp-block-group.alignfull .wp-block-cover__inner-container,.wp-block-group.alignfull .wp-block-group__inner-container .wp-block-columns{max-width:' . esc_attr($container_width) . 'px;}';
		}
		...
		if ( $accent_color && $accent_color != '#d82978' ) {
			...
			$css[] = '.woocommerce-info,.woocommerce-message,.featured-style-1 .wp-block-columns .wp-block-column,.wp-block-columns.featured-style-1 .wp-block-column,.wp-block-columns.featured-style-1 .wp-block-column{border-color:' . $accent_color . ';}';
		...
		if ( $accent_color3 && $accent_color3 != '#fbfbfb' ) {
			...
			$css[] = '@media only screen and (max-width: 1024px){.toggle-nav-open,.toggle-nav-open:hover{background:' . $accent_color3 . ';}}';
...
function cordero_block_editor_dynamic_style( $css = array() ) {
	...
	$container_width = get_theme_mod( 'container_width', '1240' );
		if ( $container_width && $container_width != '1240' ) {
			$css[] = '.wp-block,.wp-block[data-align="wide"]{max-width:' . esc_attr ($container_width ) . 'px;}';
		}
